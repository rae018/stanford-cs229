...
fileName = "brownian_overhand_fast_4-60Hz"

fileName_new = fileName + "_extended2.csv"
    ...
    for idx, row in enumerate(plots):
        if idx != 0:
            ...
            if idx <= 1: #rolling average values of each muscle signal
                ...
                for indice in range(len(muscles)):
                    fila += '0' #initial values have no divergence from rolling average
                ...
                integrada = [0 for musculo in range(len(muscles))]
                for indice in range(len(muscles)): #rolling average values of each muscle signal
                    integrada[indice] = sum(window[:,indice])/len(window[:,indice]) #rolling average for relevant entries
                    fila += str(integrada[indice]) + ","
                for indice in range(len(muscles)): #divergence from rolling average values of each muscle signal
                    #integrada = sum(window[:,indice])/len(window[:,indice]) #rolling average for relevant entries
                    #print(muscles[indice])
                    #print(integrada[indice])
                    fila += str(float(muscles[indice]) - integrada[indice])
            ...
            for header in row[-4:]:
                ctr += 1
                fila += header + " divergences from rolling average"
                if ctr < len(row[-4:]):
                    fila += ","
